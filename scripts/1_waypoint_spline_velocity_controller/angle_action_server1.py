...
class PositionController():
    def __init__(self, control_rate):
        ...
        self.force_threshold = 50 # Force in Newtons
    ...
    def update_plan(self, waypoints, duration=1.5, reset=False):
        ...
        # rospy.logerr(("update", waypoints, duration, reset))

        if reset:
            self.reset()
            self.update_plan_to_here()
            return
    ...
    def reset(self):
        ...
        rospy.logerr("reset called")
        data = pickle.load(open("/home/ashvin/ros_ws/reset2.p", "rb"))
        joints = data["joints"]
        self.limb.move_to_joint_positions(joints)
        rospy.logerr("reset done")

    def update_plan_to_here(self):
