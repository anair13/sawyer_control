...
class WSG50Gripper:
    def __init__(self):
        ...
        self._bg.daemon = True
        self._bg.start()

    def do_cmd(self, direction):
        """Input direction in [-1, 1]"""
        delta = direction * 100
        desired_gpos = np.clip(self._desired_gpos + delta, GRIPPER_CLOSE, GRIPPER_OPEN)
        self.set_gripper(desired_gpos)

    def do_open_cmd(self):
        self.set_gripper(GRIPPER_OPEN)

    def do_close_cmd(self):
