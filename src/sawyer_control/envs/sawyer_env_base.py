...
wait_for_service_flag = True # False speeds up code, is risky though [Gerrit]
...
class SawyerEnvBase(gym.Env, Serializable, MultitaskEnv, metaclass=abc.ABCMeta):
    def __init__(
        ...
        self.action_service_main = 'angle_action'       # stiff
        self.action_service_alternate = 'angle_action2' # stopping when colliding
        self.action_service = self.action_service_main 

        self.use_safety_box = use_safety_box
        #self.AnglePDController = AnglePDController(config=self.config) # PD controller not used

        self._set_action_space()
        self._set_observation_space()
        #self.get_latest_pose_jacobian_dict() # freezes environment when jacobian_server is not running
        ...
        self.pos_control_ee_orientation = self.config.POSITION_CONTROL_EE_ORIENTATION
        ...
        self.image_subscriber = rospy.Subscriber("/flat_robot_image", imagemsg, self.store_latest_image)
        self.last_image_timestamp = time.time()

        self.use_AR_tags = use_AR_tags
        # wait for services at startup instead of during operations
        if wait_for_service_flag == False:
            print("waiting for services")
            #rospy.wait_for_service('get_robot_pose_jacobian') # jacobian_server is causing errors
            rospy.wait_for_service('images')
            rospy.wait_for_service('observations')
            rospy.wait_for_service('angle_action')
            try:
                rospy.wait_for_service('angle_action2', 1.0) # comment out when no second action service is used 
            except:
                print('Service angle_action2 not running, substituting with angle_action')
                self.action_service_alternate = self.action_service_main
            rospy.wait_for_service('ik')
            if self.use_AR_tags:
                try:
                    rospy.wait_for_service('ar_tag', 1.0)
                except:
                    print('Service ar_tag not found')
    ...
    def _position_act(self, action, clip = True, orientation = None):
        joint_angles, _, ee_pos, _, _ = self.request_observation()
        self.ee_pos = ee_pos
    ...
    def _safe_move_to_neutral(self):
        for _ in range(self.config.RESET_LENGTH):
            cur_pos, cur_vel, _, _, _ = self.request_observation()
    ...
    def _check_reset_complete(self):
        ...
        _, velocities, _, _, _ = self.request_observation()
    ...
    def store_latest_image(self, data):
        ...
        self.most_recent_image = np.frombuffer(data.image, dtype=np.uint8)
        # npimg = np.array(self.most_recent_image) # .astype(np.uint8)
        # rospy.logerr(("image", self.most_recent_image.dtype, self.most_recent_image.shape))
        self.last_image_timestamp = time.time()

    def request_image(self):
        # rospy.loginfo(("returned image, stale", time.time() - self.last_image_timestamp))
        ...
        #     obs = request()
        #     rospy.logerr(("image request elapsed time", time.time() - now))
        #     npimg = np.array(obs.image) # .astype(np.uint8)
        #     rospy.logerr(("image", npimg.dtype, npimg.shape))
        #     return (
        #             obs.image
        #     )
        # except rospy.ServiceException as e:
    ...
    def get_image(self, width=84, height=84):
        ...
        if self.camera == "sawyer_head":
            # image = np.array(image).reshape(300, 500, 3)
            image = np.array(image).reshape(48, 48, 3)
            return image
    ...
    def request_observation(self):
        ...
        try:
            ...
            return (
                    ...
                    np.array(obs.endpoint_vel),
                    np.array(obs.endpoint_wrench)
            )
        except rospy.ServiceException as e:
            print(e)

    def request_angle_action(self, angles, pos):
        dist = np.linalg.norm(self.ee_pos[:3] - pos[:3])
        a = abs(sum(self.ee_pos[3:] * pos[3:])) 
        a = np.clip(a, 0.0, 1.0) # arccos is defined on [-1, 1]
        rot_angle = 2*np.arccos(a) # 0 <= rot_angle <= 3.14159
        # duration = dist/self.max_speed + rot_angle*(180/np.pi)/45
        # duration = np.clip(duration, 0.075, 10) # lower bound on duration needed when using velocity control 
        duration = 0.5
        reset = False                
        # print("duration", duration, dist) 
        # print(self.action_service)
        if wait_for_service_flag:
            rospy.wait_for_service(self.action_service)
        try:
            execute_action = rospy.ServiceProxy(self.action_service, angle_action, persistent=True)
            execute_action(angles, duration, reset)
            return None
        except rospy.ServiceException as e:
            pass

    def request_reset_angle_action(self):
        angles, _, ee_pos, _, _ = self.request_observation()
        duration = 1.5
        reset = True
        print((angles, duration, reset))
        if wait_for_service_flag:
            rospy.wait_for_service(self.action_service)
        try:
            execute_action = rospy.ServiceProxy(self.action_service, angle_action, persistent=True)
    ...
    def get_env_state(self):
        return self._get_joint_angles(), self._get_endeffector_pose()[:3]
